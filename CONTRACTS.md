...
對方半場：x ∈ [-9, 0]
z 軸（邏輯）：∈ [0, 9]，z = 0 是左邊線（站我方底線面向網），z = 9 是右邊線
Three.js Y 軸：向上（高度）
單位：公尺
```

### 呈現層水平鏡像（2026-07-05 修正）

**logic 座標維持上述定義不動**（`court.ts`、`receive.ts`、`defense.ts` 與所有測試皆活在邏輯座標，z=0/z=9 定義不變）。
原本 3D 直接以 world-z = 邏輯z 渲染，造成左手系、畫面左右與現實相反。

**修法：只在「呈現層 + 拖曳 I/O」統一做水平鏡像**

```
世界 worldZ = 9 − 邏輯z        （渲染球員/攻擊手/地面多邊形/箭頭時）
邏輯z    = 9 − worldZ          （拖曳回存 store 時反向）
```

不使用負向 scale group（會翻面法線 / 破壞光照）。實作點：
- `scene/utils.ts` `useGroundPolygonGeometry`：shape.y 用 `z − 9`（＝ −(9−z)），一次鏡像所有走此 helper 的地面多邊形（責任區塊、攔網影子、攻擊扇形）。
- `MyPlayers` / `Attackers` / `ReceivePlayers`：`group.position` 與 lerp 目標 z 用 `9 − 邏輯z`；拖曳 `onDrag` 回存 store 的 z 用 `9 − worldZ`。
- `ReceiveOverlays`：合法框中心 `9 − cz`、舉球插上箭頭起訖點 `9 − z`（箭頭指向網右＝2號位側）。
- `Court` / `Net` / `ReceiveNet`：z 對稱（邊線 z=0/9、網沿 z 0–9、所有沿-z 線橫跨全寬），無不對稱標記 → 不需改。

### 號位方向定義（鏡像後，教練 2026-07-05 確認）

站我方底線面向網，top 視角「網在畫面上方、我方半場在下方」：
```
前排（近網）左 → 右：4 - 3 - 2
後排（底線）左 → 右：5 - 6 - 1
```

| 號位 | 中文 | 前/後排 | 左/中/右 | 邏輯 z | 世界 z (=9−z) |
|------|------|---------|----------|--------|----------------|
| 4 | 左前 | 前排 | 左 | 2.0 | 7.0 |
| 3 | 中前 | 前排 | 中 | 4.5 | 4.5 |
| 2 | 右前 | 前排 | 右 | 7.0 | 2.0 |
...
### 陣型角色（教練 2026-07-04 指定，兩套體系共用）
- 1=舉球（S）、2=主攻（OH1）、3=攔中（MB1）、4=舉對/輔舉（OP）、5=主攻（OH2）、6=攔中（MB2）
- 自由球員（L）固定替換**6號位**後排攔中（`liberoId = 6`）
...
| 視角 | camera.position | camera.lookAt | 說明 |
|------|-----------------|---------------|------|
| `top` | [7, 20, 4.5] | [2, 0, 4.5] | 近俯視，網在畫面上方、我方半場在下方；配合鏡像 → 前排左→右 4-3-2、後排左→右 5-6-1。不用純正上方以免 up 退化 |
| `baseline` | [14.5, 3, 4.5] | [0, 1.5, 4.5] | 我方底線後方看向網子（左右隨鏡像自動正確） |
| `side` | [2.5, 8, 16.5] | [1, 0, 4.5] | 側面 |
| `coach` | [13, 10, 12] | [1, 0, 4.5] | 45° 教練視角（預設） |
